...
18. 当用户提到要提交至GitHub时，需要检查项目中存在的漏洞以及是否有敏感信息泄露的风险，若有则需要及时修复。执行系统临时文件清理操作，确保在清理过程中不影响所有相关客户端（包括但不限于后端、Web端、管理端等）的正常启动、功能运行及数据访问。清理前需对所有临时文件进行分类识别，排除系统运行必需文件、用户会话数据及应用程序缓存依赖文件。清理完成后，需逐一验证各客户端的启动速度、功能完整性及数据一致性，确保无异常报错或功能缺失情况发生。
18. 启动后端时，请单独启动Redis服务，确保Redis服务正常运行，该软件的位置为D:\Accounting-system\AI-server\redis-server.exe
19. PowerShell不支持&&操作符，请使用分号分隔命令。
20. 不得出现中文乱码或者格式错误的情况
