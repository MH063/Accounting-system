...
const parseDate = (dateInput: any): Date | null => {
  ...
  try {
    ...
    if (typeof dateInput === 'string') {
      ...
      // 如果解析失败，尝试处理常见的后端返回格式
      if (isNaN(date.getTime())) {
        // 处理 PostgreSQL 时间戳格式: "2024-01-15 10:30:00"
        const pgTimestampMatch = dateInput.match(/^(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2}):(\d{2})/)
        if (pgTimestampMatch) {
          date = new Date(
            parseInt(pgTimestampMatch[1]),
            parseInt(pgTimestampMatch[2]) - 1,
            parseInt(pgTimestampMatch[3]),
            parseInt(pgTimestampMatch[4]),
            parseInt(pgTimestampMatch[5]),
            parseInt(pgTimestampMatch[6])
          )
        }
      }
