...
async function testValidateTokenComplete() {
  console.log('🔍 完整测试验证令牌接口...\n');
  
  try {
    // 1. 先执行短信登录获取有效的会话令牌
    console.log('1️⃣ 执行短信登录获取会话令牌...');
    
    const loginResponse = await axios.post(`${BASE_URL}/api/auth/sms-login`, {
      phone: '13800138000',
      code: '123456'
    }, {
