...
这是一个专为寝室生活设计的智能化记账宝，采用前后端分离架构，支持多角色权限管理、实时数据同步、可视化统计分析、安全支付等核心功能，让寝室财务管理变得简单、透明、高效。系统包含完整的用户认证、寝室管理、费用记录、账单生成、支付处理和统计分析等功能模块。

**当前版本状态**: 开发完成度95%，核心功能已实现，包含完整的用户界面和业务逻辑。
...
├── components/          # 可复用组件
│   ├── __tests__/             # 组件测试文件
│   │   └── Header.search.test.js  # Header搜索功能测试
...
│   ├── BillManagement.vue.backup # 账单管理页面备份
...
**开发进度说明**:
- ✅ 已完成: 所有页面组件、布局组件、路由配置、业务服务
- ✅ 已完成: TypeScript类型定义、工具函数、样式文件
- ✅ 已完成: 组件测试框架搭建
- 🔄 预留: composables目录、middleware目录用于后续扩展
...
- **JWT身份认证**：安全的用户身份验证机制
- **权限审计日志**：完整的权限操作记录
- **生物识别认证**：支持指纹、面部识别等生物特征认证
- **两步验证**：支持TOTP二次验证和安全问题验证
...
### 前端架构
- **Vue 3.5.24** - 最新稳定版本，支持Composition API
- **TypeScript 5.9.3** - 完整类型系统支持
- **Vite 7.2.2** - 高性能构建工具
- **Vue Router 4.6.3** - 路由管理和权限控制
